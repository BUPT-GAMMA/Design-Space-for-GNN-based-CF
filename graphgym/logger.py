...
class Logger(object):
    ...
    def regression(self):
        ...
        if np.isinf(mae).any() or mae > MAX_ERR:
            mae = MAX_ERR
        mse = round(float(mean_squared_error(true, pred)), cfg.round)
        if np.isinf(mse).any() or mse > MAX_ERR:
            mse = MAX_ERR
        rmse = round(float(math.sqrt(mean_squared_error(true, pred))), cfg.round)
        if np.isinf(rmse).any() or rmse > MAX_ERR:
            rmse = MAX_ERR
    ...
    def update_stats(self, true, pred, loss, lr, time_used, params, eval_metric=0):
    ...
    def write_epoch(self, cur_epoch):
        ...
        eval_stats = {cfg.metric_best: round(self._eval_metric, cfg.round)}
        if len(set(eval_stats.keys()) & set(task_stats.keys())) != 0:
            eval_stats = {}

        if self.name == 'train':
            stats = {**epoch_stats, **eta_stats, **basic_stats, **task_stats}
        else:
            stats = {**epoch_stats, **basic_stats, **task_stats, **eval_stats}
