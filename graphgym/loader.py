...
def create_dataset():
    ...
    if os.path.isfile(split_fn):
        with open(split_fn, 'rb') as file:
            datasets = pickle.load(file)

        logging.info(f'Loading pre-stored split {split_fn}...')

    ## Create whole dataset and define the split mode accordingly
    else:
        logging.info(f'Loading newly defined split {split_fn}...')
        graphs = load_dataset()
