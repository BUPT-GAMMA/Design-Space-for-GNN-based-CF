...
def set_cfg(cfg):
    ...
    # The value of topk in ranking metrics
    cfg.topk = 20

    # is the result file of an exp is existing, skip it
    cfg.skip_existing_exp = False
    ...
    cfg.dataset.load_type = 'pointwise'

    # if srandardize the ratings
    cfg.dataset.standardization = True
    ...
    # Whether split the dataset in a customized way
    cfg.dataset.custom_split = False
    ...
    # message-passing
    cfg.dataset.edge_message_ratio = 0.81
    ...
    # non-ranking / ranking
    cfg.model.eval_type = 'non-ranking'
    ...
    #   (multi-class)
    #   - summation: use u + v followed by an nn.Linear to obtain edge embedding
    ...
    # Number of user/item components
    cfg.gnn.component_num = 1

    # Aggregation func of different components: concat/add/att
    cfg.gnn.component_aggr = 'add'
    ...
    # Type of graph conv: none, generalconv, gcnconv, sageconv, gatconv, ginconv...
    cfg.gnn.layer_type = 'customconv'

    # Stage type: 'stack', 'skipsum', 'skipconcat', 'skipmean'
    ...
    #Message func of gnn: 'identity', 'hadamard'
    cfg.gnn.msg = 'identity'

    # Whether attention mechanism is employed when aggregating neighbors
    cfg.gnn.att = False

    #Aggregation type: add, mean, none
    cfg.gnn.agg = 'add'

    # Combine func of gnn: 'identity', 'add', 'concat'
    cfg.gnn.combine = 'add'

    # Normalize adj
    cfg.gnn.normalize_adj = False

    # whether add self loop in the adj. matrix
    cfg.gnn.add_self_loop = True
...
def assert_cfg(cfg):
    ...
    if cfg.dataset.encoder_dim != cfg.gnn.dim_inner and cfg.gnn.layers_pre_mp < 1:
        cfg.gnn.layers_pre_mp = 1
        logging.warning('Layers before message passing should be >=1 if encoder_dim and gnn_dim don\'t match')
...
def update_out_dir(out_dir, fname):
    ...
    if cfg.train.auto_resume or cfg.skip_existing_exp:
